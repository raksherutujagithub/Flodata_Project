...
def load_ip_addresses(file_path):
    ...
    for ip in df['ip_address']:
        if ip not in existing_ips_set:  # Skip if IP already processed
    ...
    if new_data:
        ...
        new_df.to_sql('ip_data', con=engine, if_exists='append', index=False, method='multi')

    print("IP addresses processed and saved successfully!")
